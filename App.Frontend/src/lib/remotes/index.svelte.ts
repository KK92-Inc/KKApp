...
import { paginate, Problem, type Paginated } from '$lib/api.js';
import type {
	PathsWithMethod,
	SuccessResponse,
	ResponseObjectMap,
...
> {
	...
	/**
	 * Mark the remote function's return value as required, so that the builder will throw
	 * if the response is 404
	 * @param value
	 */
	public required(value = true) {
	...
	public paginated(): RemoteBuilder<M, TPath, TData, Paginated<TOutput>> {
	...
	public declare(): DeclareReturn<M, TData, TOutput> {
		...
		const handler = async (data: TData) => {
			...
			const result = this.isPaginated
				? paginate(output.data as unknown as Array<unknown>, output.response)
				: output.data;

			for (const fn of this.afterFns) {
				await fn(result as TOutput, data);
			}
