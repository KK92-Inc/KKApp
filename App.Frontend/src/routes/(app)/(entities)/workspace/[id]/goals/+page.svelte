...
<div class="md:container mx-auto space-y-6 px-4 py-4 lg:px-6">
	...
	<svelte:boundary>
		{@const result = await Goal.getPage({
			page: index,
		...
		<Item.Group class="flex gap-4">
			{#each result.data as goal (goal.id)}
				<Item.Goal {goal} session={{ state: "Awaiting", userId: page.data.session.userId}}>
					{#snippet actions()}
						<Button
							variant="outline"
							size="sm"
							href="/users/{page.data.session.userId}/goals/{goal.id}"
						>
							View
						</Button>
					{/snippet}
